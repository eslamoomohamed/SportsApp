...
class CountryCell: UITableViewCell {
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        if selected {
            imageView1.image = UIImage(systemName: "circle.fill")
        }else{
            imageView1.image = UIImage(systemName: "circle")

        }
    }
    
    
    func setCell(country: Countries){
        
        self.titleLabel.text = country.name_en
